const canvas = document.getElementById("tetrisCanvas");
        ...
        function collision() {
            return currentPiece.shape.some((row, dy) =>
                row.some((cell, dx) =>
                    cell && (board[currentPiece.y + dy]?.[currentPiece.x + dx] !== 0 || currentPiece.y + dy >= ROWS)
                )
            );
        }

        function mergePiece() {
            currentPiece.shape.forEach((row, dy) => row.forEach((cell, dx) => {
        ...
        function drawPiece() {
            currentPiece.shape.forEach((row, dy) => row.forEach((cell, dx) => {
                if (cell) {
                    ctx.fillStyle = "red";
                    ctx.fillRect((currentPiece.x + dx) * BLOCK_SIZE, (currentPiece.y + dy) * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE);
                    ctx.strokeStyle = "black";
                    ctx.strokeRect((currentPiece.x + dx) * BLOCK_SIZE, (currentPiece.y + dy) * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE);
                }
            }));
        }
